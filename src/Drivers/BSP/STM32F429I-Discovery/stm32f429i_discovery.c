...
/** @defgroup STM32F429I_DISCOVERY_LOW_LEVEL_Private_Defines STM32F429I DISCOVERY LOW LEVEL Private Defines
  ...
  */ 
  ...
  /**
  * @brief STM32F429I DISCO BSP Driver version number V2.1.7
  */
...
#define __STM32F429I_DISCO_BSP_VERSION_SUB2   (0x07) /*!< [15:8]  sub2 version */
...
static void SPIx_Init(void)
{
  if(HAL_SPI_GetState(&SpiHandle) == HAL_SPI_STATE_RESET)
  {
    ...
    SpiHandle.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_64; // Slow it down even more

    /* On STM32F429I-Discovery, LCD ID cannot be read then keep a common configuration */
    /* for LCD and GYRO (SPI_DIRECTION_2LINES) */
    /* Note: To read a register a LCD, SPI_DIRECTION_1LINE should be set */
    SpiHandle.Init.Direction      = SPI_DIRECTION_2LINES;
    SpiHandle.Init.CLKPhase       = SPI_PHASE_1EDGE;         // Switch back to 1EDGE
...
void GYRO_IO_Init(void)
{
  ...
  /* 1. Enable ALL relevant clocks first */
  GYRO_CS_GPIO_CLK_ENABLE();
  GYRO_INT_GPIO_CLK_ENABLE();
  DISCOVERY_SPIx_CLK_ENABLE(); // Force SPI clock on

  /* 2. Configure Chip Select (PC1) - THIS IS CRITICAL */
  GPIO_InitStructure.Pin = GYRO_CS_PIN;
  GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStructure.Pull = GPIO_PULLUP; // Changed from NOPULL to ensure it stays HIGH
  GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH; // Max speed for clean edges
  HAL_GPIO_Init(GYRO_CS_GPIO_PORT, &GPIO_InitStructure);
  
  /* Force it High immediately to avoid bus conflict with LCD */
  HAL_GPIO_WritePin(GYRO_CS_GPIO_PORT, GYRO_CS_PIN, GPIO_PIN_SET);
  
  /* 3. Configure Interrupt pins */
